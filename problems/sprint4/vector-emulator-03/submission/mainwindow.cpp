...
void MainWindow::on_btn_resize_clicked() {
    ...
    if (ok && new_size >= 0 && new_size <= 1000) {
        vector_model_.items.resize(new_size);
        vector_model_.iterator = vector_model_.items.begin();
        ApplyModel();
    } else {
        QMessageBox::warning(this, "Invalid size", "Please enter a valid size between 0 and 1000");
    }
}

void MainWindow::on_btn_reserve_clicked() {
    bool ok;
    int new_capacity = ui->txt_capacity->text().toInt(&ok);
    
    if (ok && new_capacity >= 0 && new_capacity <= 1000) {
        vector_model_.items.reserve(new_capacity);
        vector_model_.iterator = vector_model_.items.begin();
        ApplyModel();
    } else {
        QMessageBox::warning(this, "Invalid capacity", "Please enter a valid capacity between 0 and 1000");
    }
}
